use std::{io::{BufReader, BufRead, BufWriter, Write}, fs::File};

fn main() -> anyhow::Result<()> {
    let mut reader = BufReader::new(
        File::open("test.txt")?
    );
    let mut proxy_writer = BufWriter::new(
        File::create("proxy.txt")?
    );
    let mut white_writer = BufWriter::new(
        File::create("white.txt")?
    );
    ...
    loop {
        ...
        if line.chars().all(|ch| ch.is_numeric() || ch == '.' || ch == ':')  {
            continue; // IP
        }

        if !line.chars().all(|ch| ch.is_ascii_alphanumeric() || ch == '_' || ch == '-' || ch == '.') {
